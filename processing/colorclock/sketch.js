...
// set by querystring 'is' parameter
var isArtcast = false;
var debug = false;
...
function setup() {
    ...
    if(getParameterByName("is") == "debug") {debug = true; }
  if(isArtcast) {
...
background(0);
  ...
  // show secon hand if debugging
  if(debug) line(cx, cy, cx + cos(s) * secondsRadius, cy + sin(s) * secondsRadius);
...
if(debug) 
  n=s; // every second, on the second hand
else
...
var cSize;
  ...
  for(var td in allData) {
      ...
      var transp = ne/maxDots*200;
      if(allData.length<maxDots) transp = 200;
